...
def gen_filename():
    while True:
        a = random_key(strings = ascii_letters) + '.png'
        if a not in  listdir(): return secure_filename(a)
...
def generate():
    ...
    if face.filename.split('.')[1] not in ['png', 'jpg', 'jpeg'] or image.filename.split('.')[1] not in ['png', 'jpg', 'jpeg']: return {'message': 'file type must be one of"png", "jpg", "jpeg"'}, 400
    ...
    if not len(faces): return {'message': 'Face not found'}, 412
